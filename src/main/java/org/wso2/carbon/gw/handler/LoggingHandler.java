...
package org.wso2.carbon.gw.handler;
 ...
 */
@Component(
        name = "org.wso2.carbon.gw.handler.LoggingHandler",
        immediate = true,
...
public class LoggingHandler implements MessagingHandler {
    ...
    public void invokeAtSourceRequestSending(CarbonMessage carbonMessage) {
        log.info("headers in request receiving to engine: " + formatHeader(carbonMessage.getHeaders()));
    }

    @Override
    public void invokeAtTargetRequestReceiving(CarbonMessage carbonMessage) {
        log.info("headers in request sending from engine: " + formatHeader(carbonMessage.getHeaders()));
    }
    ...
    public void invokeAtTargetResponseSending(CarbonMessage carbonMessage) {
        log.info("headers in response receiving to engine: " + formatHeader(carbonMessage.getHeaders()));
    }

    @Override
    public void invokeAtSourceResponseReceiving(CarbonMessage carbonMessage) {
        log.info("headers in response sending from engine: " + formatHeader(carbonMessage.getHeaders()));
    }
